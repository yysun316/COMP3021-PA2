...
public class QueryOnMethod {
     ...
     * in current module {@link QueryOnMethod#module}
     *
     * @param funcName the name of the function to be queried
     * @return results List of strings where each represents a comparison expression,
     * in format, lineNo:colOffset-endLineNo:endColOffset
    ...
    public Function<String, List<String>> findEqualCompareInFunc = (funcName) -> {
        ...
        module.filter(astElement -> astElement instanceof FunctionDefStmt &&
                        funcName.equals(((FunctionDefStmt) astElement).getName()))
                .forEach(func -> func.filter(el -> el instanceof CompareExpr)
                        .forEach(compareExpr -> ((CompareExpr) compareExpr).getOps().forEach(op -> {
                            if ("Eq".equals(op.getOperatorName()))
                                res.add(compareExpr.getLineNo() + ":" + compareExpr.getColOffset() +
    ...
    /**
     * TODO `findFuncWithBoolParam` find all functions that use boolean parameter as if condition in
     * current module {@link QueryOnMethod#module}
     *
     * @param null
    ...
    public Supplier<List<String>> findFuncWithBoolParam = () -> {
        ...
        List<String> tmp = new ArrayList<>(); // storing all the test id.
        // Find all the parameter name in test
        module.filter(astElement -> astElement instanceof FunctionDefStmt)
                .forEach(func -> func.filter(child -> child instanceof IfStmt)
                        .forEach(ifStmt -> {
...
//        System.out.println(tmp);
        ...
        module.filter(astElement -> astElement instanceof FunctionDefStmt)
                .forEach(func -> func.filter(child -> child instanceof ASTArguments)
                        .forEach(args -> args.filter(child -> child instanceof ASTArguments.ASTArg &&
                                        ((ASTArguments.ASTArg) child).getAnnotation() != null)
                                .forEach(arg -> {
    ...
    public Function<String, List<String>> findUnusedParamInFunc = (funcName) -> {
        ...
        module.filter(astElement -> astElement instanceof FunctionDefStmt
                        && funcName.equals(((FunctionDefStmt) astElement).getName())) // get the function
                        ...
                        .forEach(e -> {
                            ...
                            if ("Load".equals(type)) {
                                if (storeNames.containsKey(id)) {
                                    ...
                                    if (lineNo > expr.getLineNo() ||
                                            lineNo == expr.getLineNo() && colOffset > expr.getColOffset())
                                        loadNames.add(id);
                                } else {
                                    loadNames.add(id);
                                }
...
//        System.out.println(loadNames);
        ...
        module.filter(astElement -> astElement instanceof FunctionDefStmt &&
                ...
                .forEach(func -> func.filter(child -> child instanceof ASTArguments)
                        .forEach(args -> args.filter(child -> child instanceof ASTArguments.ASTArg)
                                .forEach(arg -> {
                                    ASTArguments.ASTArg astArg = (ASTArguments.ASTArg) arg;
                                    if (!loadNames.contains(astArg.getArg()))
                                        names.add(((ASTArguments.ASTArg) arg).getArg());
                                })));
        return names;
    };
     ...
     * by functions other than B in current module {@link QueryOnMethod#module}
     *
    ...
    public BiPredicate<String, String> answerIfACalledB = (funcNameA, funcNameB) -> {
        // Get funcA
        ArrayList<ASTElement> funcA = module.filter((astElement -> astElement instanceof FunctionDefStmt &&
                funcNameA.equals(((FunctionDefStmt) astElement).getName())));
        if (funcA.isEmpty())
            return false;
        ...
        for (String name : calledFuncNames) {
            ...
            ArrayList<ASTElement> curFunc = module.filter(astElement -> astElement instanceof FunctionDefStmt &&
                    name.equals(((FunctionDefStmt) astElement).getName()));
            if (!curFunc.isEmpty())
                curFunc.get(0).filter(astElement -> astElement instanceof CallExpr)
                        .forEach(callExpr ->
                                calledFuncNames.add(((CallExpr) callExpr).getCalledFuncName())
                        );
        }
